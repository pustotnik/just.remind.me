...
jrm::Application theApp;

void stopWorkOnSignal(int signal)
{
    theApp.sayStop();
}
...
int main(int argc, char** argv)
{
    ...
    sighandler_t sigHandler = stopWorkOnSignal;
    signal(SIGINT,  sigHandler);
    signal(SIGTERM, sigHandler);

    // run application
    jrm::CmdLineArgs cmdLineArgs(argv, argv + argc);
    return theApp.run(cmdLineArgs);
}
